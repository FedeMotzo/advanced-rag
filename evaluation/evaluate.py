"""
Script di valutazione RAGAS per la pipeline RAG.

Confronta le strategie di chunking (Recursive vs Semantic) usando le metriche RAGAS.
...
def run_pipeline(orchestrator: RAGOrchestrator, questions: list) -> list:
    ...
    for item in questions:
        ...
        print(f"    🔍 {question[:70]}...")
        result = orchestrator.ask(question, source="text")
        results.append({
            "question": question,
...
def evaluate_strategy(strategy, strategy_name: str, gold_data: list, evaluator_llm, evaluator_embeddings, reranker: bool = False, hybrid_search: bool = False) -> pd.DataFrame:
    ...
    retrieval_mode = "Hybrid" if hybrid_search else "Semantic"
    ...
    # 1. Setup orchestrator con collection specifica per strategia
    # Nota: la collection dipende SOLO dalla strategia di chunking, non dal retrieval/reranking
    ...
    if n_chunks == 0:
        print(f"  📂 Ingestion documenti (collection vuota)...")
        for f in DOC_FILES:
            n_chunks += orchestrator.ingest(f)
        print(f"     Chunks creati: {n_chunks}")
    else:
        print(f"  ♻️  Riutilizzo chunks esistenti: {n_chunks}")
...
def print_comparison(all_results: pd.DataFrame, gold_data: list):
    ...
    metric_cols = [c for c in all_results.columns
                   if c not in ("strategy", "n_chunks", "user_input", "response", "retrieved_contexts", "reference")]
    ...
    # Ordina per Answer Relevancy (solitamente la più critica)
    if "answer_relevancy" in summary.columns:
        summary = summary.sort_values("answer_relevancy", ascending=False)

    print(f"\n{'Configurazione':<45} {'Chunks':>6}", end="")
    for col in metric_cols:
        print(f"  {col:>18}", end="")
    print()
    print("─" * (53 + 20 * len(metric_cols)))

    for strategy_name, row in summary.iterrows():
        print(f"{strategy_name:<45} {int(row['n_chunks']):>6}", end="")
        for col in metric_cols:
            val = row[col]
            emoji = "✅" if val >= 0.7 else "⚠️" if val >= 0.4 else "❌"
            print(f"  {emoji} {val:>14.3f}", end="")
        print()

    # --- Dettaglio per domanda ---
    print(f"\n{'─' * 80}")
    print("  📝 Dettaglio per domanda (Top 3 configs)")
    print(f"{'─' * 80}")

    top_3_strategies = summary.index[:3]
    ...
    for i, item in enumerate(gold_data):
        ...
        rows = all_results[
            (all_results["user_input"] == q) & 
            (all_results["strategy"].isin(top_3_strategies))
        ]
        
        for _, row in rows.iterrows():
            print(f"    [{row['strategy']}]", end="")
            for col in metric_cols:
                print(f"  {col}={row[col]:.3f}", end="")
            print()
...
def main():
    ...
    print(f"\n📋 Dataset gold: {len(gold_data)} domande")
    ...
    # Genera le 8 configurazioni
    configs = []
    for strategy, name in [(RecursiveChunkingStrategy(), "Recursive"), (SemanticChunkingStrategy(), "Semantic")]:
        for hybrid in [False, True]:
            for rerank in [False, True]:
                configs.append((strategy, name, rerank, hybrid))
